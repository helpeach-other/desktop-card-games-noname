...
class JieDao extends CardEntry {
  name = '借刀杀人'
  image = 'src/assets/images/cards/standard/jie-dao.png'
  type = CardType.Trick
  description = '出牌阶段，对装备区里有武器牌且有使用【杀】的目标的一名其他角色使用。令其对你指定的一名角色使用一张【杀】，否则将其装备区里的武器牌交给你。'
}

class WuXie extends CardEntry {
  name = '无懈可击'
...
class ShanDian extends CardEntry {
  name = '闪电'
  image = 'src/assets/images/cards/standard/shan-dian.png'
  type = CardType.Delay
  description = '出牌阶段，对自己使用。若判定结果为黑桃2~9，则目标角色受到3点雷电伤害。若判定不为黑桃2~9，将之移动到下家的判定区里。'
}

class JueDou extends CardEntry {
  name = '决斗'
  image = 'src/assets/images/cards/standard/jue-dou.png'
  type = CardType.Trick
  description = '出牌阶段，对一名其他角色使用。由其开始，其与你轮流打出一张【杀】，直到其中一方未打出【杀】为止。未打出【杀】的一方受到另一方对其造成的1点伤害。'
}

class NanMan extends CardEntry {
  name = '南蛮入侵'
...
class ShunShou extends CardEntry {
  name = '顺手牵羊'
...
class TaoYuan extends CardEntry {
  name = '桃园结义'
...
class WanJian extends CardEntry {
  name = '万箭齐发'
...
class WuZhong extends CardEntry {
  name = '无中生有'
...
class WuGu extends CardEntry {
  name = '五谷丰登'
  image = 'src/assets/images/cards/standard/wu-gu.png'
  type = CardType.Trick
  description = '出牌阶段，对所有角色使用。（选择目标后）你从牌堆顶亮出等同于目标数量的牌，每名目标角色获得这些牌中（剩余的）的任意一张。'
}

class HanBing extends CardEntry {
  name = '寒冰剑'
...
class RenWang extends CardEntry {
  name = '仁王盾'
...
class JueYing extends CardEntry {
  name = '绝影'
...
class ZhuaHuang extends CardEntry {
  name = '爪黄飞电'
...
class QingLong extends CardEntry {
  name = '青龙偃月刀'
...
class GuanShi extends CardEntry {
  name = '贯石斧'
...
class FangTian extends CardEntry {
  name = '方天画戟'
...
class CiXiong extends CardEntry {
  name = '雌雄双股剑'
...
class ZhangBa extends CardEntry {
  name = '丈八蛇矛'
...
class ZhuQue extends CardEntry {
  name = '朱雀羽扇'
...
class QingGang extends CardEntry {
  name = '青釭剑'
...
export const standardCardMap: Record<string, typeof CardEntry> = {
  Sha,
  Shan,
  Tao,
  GuoHe,
...
export const standardCards: [CardSuit, number, typeof CardEntry][] = [
